...
public class GraphSearch {
	...
	public void BFS(int nStart) {
		// Iterative BFS places vertices in a queue. When we pull a vertex out
		// of
		// the queue, we process it (in this case, print it to the screen), then
		// place all its unvisited neighbors in the queue. We mark each vertex
		// as
		// visited as it goes into the queue, because that ensures we never
		// place
		// a vertex into the queue more than once. (That can significantly
		// reduce
		// the space complexity of this algorithm when dealing with a large,
		// dense
		// graph.)
		...
		while (!objQueue.isEmpty()) {
			...
			// whether we had found it. If so, we would terminate the BFS.
			// However,
			// since the goal of this BFS method is simply to print all nodes of
			// a
	...
	// Wrapper to our recursive DFS method. This one sets up the 'visited'
	// array.
	...
	private void DFS(int nNode, boolean[] bVisited) {
		...
		// If we were searching for a particular vertex (a "goal"), this is
		// where we
		// would check whether we have found the goal. If so, we would terminate
		// the
		// DFS. However, since the purpose of this particular method is simply
		// to
		// print all vertices in DFS order, we just print this vertex and move
		// on.
		System.out.println(nNode);
